...
const _sfc_main = {
  ...
  methods: {
    ...
    getSearchTime(year, month) {
      ...
      let { start, end } = utils_timeUtils.getMonthStartAndEnd(year, month);
      this.timeObj = {
        start_time: utils_timeUtils.formatTimeBase(start, "{y}-{m}-{d}"),
        end_time: utils_timeUtils.formatTimeBase(end, "{y}-{m}-{d}")
      };
    },
    search(data) {
      this.searchObj = data;
