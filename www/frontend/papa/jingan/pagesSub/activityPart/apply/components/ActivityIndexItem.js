...
const _sfc_main = {
  ...
  computed: {
    ...
    buttonDiaabled() {
      return this.showInfo.status != 2;
    }
  },
  methods: {
    formatData(data) {
      data.images_url = data.images_url || [];
      let obj = {
        banner: data.images_url.length > 0 ? data.images_url[0] : this.defaultImgUrl,
        name: data.name,
        address: data.address,
        applyTimeStr: data.applyTimeStr ? data.applyTimeStr : this.filterTime(
          utils_timeUtil.formatTimeBase(data.apply_start_time),
          utils_timeUtil.formatTimeBase(data.apply_end_time)
        ),
...
if (!Array) {
  ...
  (_component_paTag + _easycom_uv_icon2 + _easycom_uv_line_progress2)();
}
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    f: common_vendor.f($data.showInfo.tagList, (item, k0, i0) => {
      return {
...
const Component = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-8a9269f1"]]);
