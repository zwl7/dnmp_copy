...
const _sfc_main = {
  ...
  data() {
    return {
      ...
      team_min_num: 0,
      // 队伍最小人数 0=不限制
      is_diff_team_repeat: 2
      // 是否允许不同队伍重复报名  1=是，2=否
    };
  },
  computed: {
    headerCustomStyle() {
  ...
  onLoad(options) {
    common_vendor.index.$log.info("赛事活动报名参数options", options);
    ...
    this.gender_limit = options.gender_limit;
    this.team_max_num = options.team_max_num;
    this.team_min_num = options.team_min_num;
    this.is_diff_team_repeat = options.is_diff_team_repeat;
  ...
  methods: {
    ...
    getCommonUsrData(data) {
      return __async(this, null, function* () {
        ...
        this.getInsuranceStatus();
        let flag = yield this.checkBeforeAdd();
        if (!flag) {
          this.selectUserInfo = {};
          this.insurance_current_member = "";
          this.is_buy_insurance = false;
          this.insurance_buy_num = 0;
          return;
        }
    ...
    checkBeforeAdd() {
      return __async(this, null, function* () {
        return new Promise((resolve, reject) => __async(this, null, function* () {
          ...
          let res = yield apis_match.checkBeforeAdd(form);
          if (res.code !== 200) {
