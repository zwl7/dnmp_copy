...
const _sfc_main = {
  ...
  data() {
    return {
      ...
      shareIcon: common_assets.shareI,
  ...
  onLoad(options) {
    if (options.member_units_id) {
      let form = {
        member_units_id: options.member_units_id
      };
  ...
  methods: {
    ...
    toCollect() {
      common_vendor.index.$uv.throttle(() => __async(this, null, function* () {
        ...
        let params = {
          ...
          topic_id: this.form.member_units_id,
    ...
    getDetail() {
      return __async(this, null, function* () {
        let res = yield apis_common.getWxUnitDetail(this.form);
        if (res.code == 200) {
          res.data.des = this.$formatRichText(res.data.des, "暂无内容");
          this.info = res.data;
          if (this.info.cover) {
            this.info.cover = this.info.cover.split(",");
            console.log(this.info.cover, "cover");
          } else {
            this.info.cover = [
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    ...
    b: common_vendor.f($data.info.cover, (item, index, i0) => {
      return {
        a: item,
        b: common_vendor.o(($event) => $data.previewImage(item), index),
        c: index
      };
    }),
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-c07d57ff"]]);
