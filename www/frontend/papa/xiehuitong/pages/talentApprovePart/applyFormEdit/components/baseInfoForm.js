    ...
    formData: {
      handler(newVal) {
        if (newVal) {
          ...
          if (newVal.level_info_json_array) {
            let levelMap = {
              "-1": "国际级",
              0: "国家级",
              1: "一级",
            ...
            let data = newVal.level_info_json_array.map((e) => {
              return {
                label: e.sport_tag_id_str,
                value: e.common_type_id,
                level: e.level,
                level_str: levelMap[e.level]
              };
            });
            let sportTagConfirmData = {
              selected: data
            };
            this.sportTagConfirm(sportTagConfirmData, true);
          }
        }
      }
    }
    ...
    sportTagConfirm(data, isEdit) {
      ...
      try {
        ...
        selected.map((item) => {
          ...
          addForm[key] = isEdit ? item.level : null;
          addForm[`${key}_str`] = isEdit ? item.level_str : null;
        });
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return {
    a: common_vendor.sr("paForm", "808be580-0"),
    b: common_vendor.p({
    ...
    c: common_vendor.o((...args) => $options.validate && $options.validate(...args))
  };
}
