...
const _sfc_main = {
  ...
  methods: {
    confirm() {
      ...
      let tagObj = {
        tag_ids: this.selected.toString(),
      ...
      console.log(tagObj);
      common_vendor.index.$emit("TAGOBJ", tagObj);
    ...
    handleSelect(item) {
      if (this.isSelected(item.id)) {
        this.selected = this.selected.filter((selectItem) => item.id != selectItem);
    ...
    getData() {
      return __async(this, null, function* () {
        ...
        const tagGroup = utils_tree.formatTreeOption(res.data.list, [
          "name",
          "tag_group_id",
          "tag_id_str",
          "tag_id",
        ...
        this.dataList = utils_tree.flattenTree(tagGroup)[1];
        this.tempList = utils_tree.flattenTree(tagGroup)[1];
      });
    }
  }
};
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return {
    a: common_vendor.f($data.selected, (item, index, i0) => {
      return {
        a: common_vendor.o(($event) => $options.handleSelect({
          id: item
        }), index),
        b: "5a0f34ef-1-" + i0 + ",5a0f34ef-0",
        c: common_vendor.p({
    ...
    e: common_vendor.f($data.tempList, (item, index, i0) => {
      return common_vendor.e({
        ...
        e: "5a0f34ef-5-" + i0 + "," + ("5a0f34ef-4-" + i0),
        f: common_vendor.p({
          title: item.label
        }),
        g: index,
        h: "5a0f34ef-4-" + i0 + ",5a0f34ef-3"
      });
    }),
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-5a0f34ef"]]);
