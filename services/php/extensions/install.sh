...
installExtensionFromTgzSwoole()
{
    ...
    mkdir ${extensionName}
    tar -xf ${tgzName}.tgz -C ${extensionName} --strip-components=1
    ( cd ${extensionName} && phpize && ./configure --enable-openssl --enable-sockets --enable-swoole-json --enable-http2 && make ${MC} && make install )
...
if [[ -z "${EXTENSIONS##*,swoole,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        installExtensionFromTgzSwoole swoole-4.8.13
    else
